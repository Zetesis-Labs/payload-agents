...
export interface SyncOptions {
  /**
   * Bypass the `reuseEmbeddingsWhenContentUnchanged` optimization and run
   * the full chunk + upsert path even when the content hash matches.
   */
...
export const syncDocumentToIndex = async (
  ...
  const syncer = new DocumentSyncer(adapter, collectionSlug, tableName, tableConfig, options)
...
export class DocumentSyncer {
  ...
  /**
   * Update only metadata fields on existing indexed documents (no re-chunk,
   * no re-embed at the backend). Uses direct update for single docs,
   * filter-based batch update for chunks.
   */
