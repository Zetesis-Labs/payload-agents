...
class AgentRegistry:
    ...
    def _compose_instructions(cfg: dict[str, Any]) -> str:
        ...
        Taxonomies and searchCollections are appended as structured hints so the
        agent knows which MCP filters to apply without hard-coding them in tool args.
        """
        parts: list[str] = []
        ...
        if isinstance(taxonomies, list):
            ...
            for item in taxonomies:
                if isinstance(item, dict) and isinstance(item.get("slug"), str):
                    ...
                    slugs.append(item)
            if slugs:
                parts.append(f"[RAG filter: taxonomy_slugs={','.join(slugs)}]")
